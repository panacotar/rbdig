require 'test_helper'

class ResponseTest < Minitest::Test
  def test_header_parse
    res = RbDig::Response.new(TestConstants::GTLD_DNS_REPLY)
    query_id = TestConstants::GTLD_DNS_REPLY[0..1].unpack1('n')
    assert_equal query_id, res.header[:query_id]
    assert_equal 1, res.header[:qd_count]
  end

  def test_body_parse
    res = RbDig::Response.new(TestConstants::GTLD_DNS_REPLY)
    assert_equal 'example.com', res.body[:question]
    assert_equal 1, res.body[:q_type]
  end

  def test_answers
    res = RbDig::Response.new(TestConstants::AUTH_DNS_REPLY)
    assert_equal 2, res.answers.size
  end

  def test_infinite_loop_protection
    assert_raises(RbDig::DNSMessageError) do
      RbDig::Response.new(TestConstants::INFINITE_LOOKUP_DNS_REPLY)
    end
  end

  def test_invalid_label_length
    assert_raises(RbDig::DNSMessageError) do
      RbDig::Response.new(TestConstants::INVALID_LENGTH_LABEL)
    end
  end
end

module TestConstants
  AUTH_DNS_REPLY = "".freeze

  GTLD_DNS_REPLY = "".freeze

  INFINITE_LOOKUP_DNS_REPLY = "\x00\x01\x80\x00\x00\x01\x00\x00\x00\x0d\x00\x0e\x07\x65\x78\x61\x6d\x70\x6c\x65\x03\x63\x6f\x6d\x00\x00\x01\x00\x01\xc0\x1d".freeze

  INVALID_LENGTH_LABEL = "\x00\x01\x80\x00\x00\x01\x00\x00\x00\x0d\x00\x0e\x41\x65\x78\x61\x6d\x70\x6c\x65".freeze
end
